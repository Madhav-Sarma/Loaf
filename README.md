...
- **Run locally (client):**
	...
	- Copy example env: `client/.env.example` -> `client/.env` and set `VITE_SERVER_URL`.
	- From `client` run:
...
- **Build (production):**
	...
	- Client: from `client` run `npm run build` (this runs TypeScript build then Vite build).
	- Server: from `server` run `npm run build` (TypeScript) then `npm start` to run `dist/server/index.js`.

**Progressive Web App (PWA)**

- `manifest.json` is generated by `vite-plugin-pwa` from `client/vite.config.ts` and provides metadata the browser uses to show an "install" prompt. It contains `name`, `short_name`, `icons`, `start_url`, and `display: standalone` so the app opens like a native app.

- A service worker is generated by `vite-plugin-pwa` into `client/dist/sw.js`. A service worker is a small script that runs in the background (separate from the web page) and can intercept network requests and cache responses. Our worker caches a small app shell (index, icon, manifest) on install and serves cached assets when offline.
...
- Client (Vite): any env keys exposed to the browser must be prefixed with `VITE_`, e.g. `VITE_SERVER_URL`. Vite in build time injects these into `import.meta.env` and makes them available to client code.
...
1. Frontend (Vercel)
	...
	- Set env var `VITE_SERVER_URL` in Vercel to your backend URL.
...
3. Connect frontend to server
	...
	- After backend is deployed, set the frontend env var `VITE_SERVER_URL` to the backend public URL.
	- Make sure Socket.IO client connects to that URL (e.g., `const socket = io(import.meta.env.VITE_SERVER_URL)` in client code).
...
- Add PNG icons to `client/public/icons/` and update the PWA manifest config in `client/vite.config.ts` to reference them.
- Wire `VITE_SERVER_URL` into the client's socket initialization (currently socket connection uses a relative URL). 
